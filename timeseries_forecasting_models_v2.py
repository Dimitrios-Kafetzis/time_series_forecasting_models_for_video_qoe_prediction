...
Description:
    This script implements several models for time series forecasting using TensorFlow.
    It includes the following model architectures:
        - LSTM
        - GRU
        - Transformer
        - Linear (a simple linear regressor as a baseline)
    The purpose is to predict the QoE (Quality of Experience) value for network data.
    The input dataset is composed of JSON files in two supported formats:
    ...
    Usage Examples:
      1. Train an LSTM model on a regular dataset:
         $ python3 timeseries_forecasting_models_v2.py --data_folder ./mock_dataset --model_type lstm --seq_length 5 --epochs 20 --batch_size 16

      2. Train a GRU model on an augmented dataset (without extra statistics):
         $ python3 timeseries_forecasting_models_v2.py --data_folder ./augmented_dataset --model_type gru --seq_length 5 --epochs 20 --batch_size 16 --augmented

      3. Train a Transformer model on an augmented dataset and use extra statistical features:
         $ python3 timeseries_forecasting_models_v2.py --data_folder ./augmented_dataset --model_type transformer --seq_length 5 --epochs 20 --batch_size 16 --augmented --use_stats

      4. Train a Linear regressor as a baseline on a regular dataset:
         $ python3 timeseries_forecasting_models_v2.py --data_folder ./mock_dataset --model_type linear --seq_length 5 --epochs 20 --batch_size 16
         $ python3 timeseries_forecasting_models_v2.py --data_folder ./augmented_dataset --model_type linear --seq_length 5 --epochs 20 --batch_size 16 --augmented --use_stats


      5. Perform automated hyperparameter tuning (example with LSTM on a regular dataset):
...
def build_lstm_model(seq_length, feature_dim, hidden_units=50, num_layers=2, dropout_rate=0.2,
    ...
    # Optionally, you could add a GlobalPooling layer here for further architectural improvements.
...
def build_gru_model(seq_length, feature_dim, hidden_units=50, num_layers=2, dropout_rate=0.2,
    ...
    for i in range(num_layers):
        ...
        model.add(gru_layer)
    # Optionally add a pooling layer before the Dense layers if desired.
    model.add(Dense(25, activation='relu'))
    model.add(Dense(1))
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=learning_rate), loss='mse')
    return model
...
def build_linear_model(seq_length, feature_dim, learning_rate=0.001):
    """Build and compile a simple Linear Regressor as a baseline model.
       It flattens the input sequence and applies a Dense layer with linear activation.
    """
    model = Sequential()
    model.add(Flatten(input_shape=(seq_length, feature_dim)))
...
def main():
    ...
    parser.add_argument("--model_type", type=str, default="lstm",
                        ...
                        help="Type of model to train: lstm, gru, transformer, or linear.")
    ...
    # Load data and determine feature columns.
    if args.augmented:
    ...
    # Normalize all numeric columns (features + QoE)
    norm_cols = feature_cols + ["QoE"]
    scaler = MinMaxScaler()
    df[norm_cols] = scaler.fit_transform(df[norm_cols])
    
    # Create sequences: each sequence of length 'seq_length' with target QoE at the next time step.
    ...
    # Split into training and test sets (maintaining sequential order)
    ...
    # Automated hyperparameter tuning if enabled (for non-linear models).
    if args.tune and args.model_type != "linear":
        ...
        # If model_type is "linear", we ignore tuning.
